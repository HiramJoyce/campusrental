...
// ---Retro Button---
//
var buttons = document.querySelectorAll('.btn-retro');

for(var i = 0; i < buttons.length; i++) {
  // Click
  buttons[i].addEventListener('mousedown', function() {
    this.classList.add('btn-retro-active');
  });
  buttons[i].addEventListener('mouseup', function() {
    this.classList.remove('btn-retro-active');
  });

  // Hover
  buttons[i].addEventListener('mouseleave', function() {
    this.classList.remove('btn-retro-center', 'btn-retro-right', 'btn-retro-left', 'btn-retro-active');
  });
  ...
  buttons[i].addEventListener("mousemove", function(e) {
    ...
    if(myPosX < (leftOffset + .3 * btnWidth) ) {
      newClass = 'btn-retro-left'
    } else {
      // if on right 1/3 width of btn
      if(myPosX > (leftOffset + .65 * btnWidth) ) {
        newClass = 'btn-retro-right';
      } else {
        newClass = 'btn-retro-center';
      }
    }
    // remove prev class
    var clearedClassList = this.className.replace(/btn-retro-center|btn-retro-right|btn-retro-left/gi, "").trim();
